...
c----------------------------------------------------------------------
      ...
      implicit real*8(a-h,o-z)
      parameter(mm=600,nm=400,
     &          ntm=2000,npc=100000,nss=400000,msp=1200)
c     &          ntm=2000,npc=100000,nss=400000,npl=4000,msp=1200)

c---neccton
      character dbg1(1000)
      character string2(2),string3(3) ! debugging on Juno
c      parameter (necctonH0=24.d0,necctonD=0.01d0)
...
c----------------------------------------------------------------------
        ...
        enddo
        read(41,*) numfiles
        write(6,'(a13,i4)') '*** numfiles=',numfiles
        do n=1,numfiles+1
          read(41,'(a8)') indate(n)
        write(6,'(a10)') ('|'//indate(n)//'|')
...
c----------------------------------------------------------------------
         ...
         else if (icurrents.eq.14) then
c ---- neccton Juno debugging
       ifcstfn(i) = 1

cc	      do k=1,3
c		    read(40,'(a11,i2)') fn(k),ifcstfn(i)
c		    read(40,'(a11,a2)') fn(k),string2
cc		    read(40,'(a11,a3)') fn(k),string3
cc        write(6,'(a11,a5)') fn(k),('|'//string3//'|')
cc		    ifcstfn(i) = 1
		    
cc              enddo
              
              fcstfn(i) = 'medf'//indate(i)(1:6)//'24.opa'
...
c            fcstfn(i) = 'sici'//fnsim(i)(9:14)//'12.sim'
	...
	if(iwind.eq.3.or.iwind.eq.4.or.iwind.eq.5.or.iwind.eq.27.
     &or.iwind.eq.25) then
...
c --- neccton2
	  nwfcst = numfiles+1
ccc	  read(40,'(i3)') nwfcst
ccc      write(6,'(a11,i3)') '*** nwfcst=',nwfcst
	  do i=1,nwfcst
ccc		read(40,*) wfcstfn(i),iwfcstfn(i)
      iwfcstfn(i) = 1
      wfcstfn(i) = 'ecm_'//indate(i)(1:6)//'.ecm'
      write(6,'(a14,i2)') wfcstfn(i),iwfcstfn(i)
...
c added by M. De Dominicis
          ...
          fcstcurdir='fcst_data/C24/'
c          fcstcurdir='fcst_data/MyO/'
...
c      if(icencr.ne.0) then
c        open(50,file='data/'//regn1//'_.bath',status='old')
c	else
c        open(50,file='data/'//regn1//'.bath',status='old')
c      endif
c-------- neccton
...
c          dist = 1.5d0 * tcomp                ! distance in NM
          dist = 1.0d0 * tcomp                ! distance in NM neccton
...
c      if(ispill.gt.0) write(6,602) vlunit
      ...
      if(icurrents.eq.14.and.icurrents.eq.76) write(6,645)
      ...
      if(iwind.eq.27.or.iwind.eq.25) write(6,713)
  ...
  770 format(/'Spill positions will be corrected from observations')
      ...
      if(icurrents.eq.14.or.icurrents.eq.76) write(90,645)
      ...
      if(iwind.eq.27.or.iwind.eq.25) write(90,713)
...
c
      do 94 nst=1,maxst
        ...
        if(timehr.le.necctonH0) then
          fDEGRAD = 1.0
        else
          fDEGRAD = 1.0-(fnecctonD*delt/24)
c          write(6,*) nst,'delt=',delt,'nD=',fnecctonD,'nDEGRAD=',fDEGRAD
        end if
...
c	  dist = 1.5 * (tcomp - timehr)
	  	  dist = 1.0 * (tcomp - timehr)	! neccton
...
c----------------------------------------------------------------------
     ...
     &.or.iwind.eq.27.or.iwind.eq.25) then
...
c--------------------------------------------------------------------
        if(istoke.eq.0) then
c-----neccton
c       write(6,*) 'ISTOKE= 0 NO STOKE DRIFT CALCULATION'
...
c-----neccton bug         
          if ((probd.gt.0.01d0).or.(probd.le.0)) then
            write(6,*) '*** probd ',probd,' ---> ',(probdsum/probdnum)
c            probd=1e-3
            probd=probdsum/probdnum 
          else 
            probdsum = probdsum+probd
            probdnum = probdnum+1
          endif  
...
ccccccccc       write(6,*) 'probd',l1,l2,probd
          do 66 i=l1,l2
            c1p(i)=(c1i-frace)*bpp
      ...
      if (i.eq.99999) then 
        write(6,*) 'after probd ',is(1),is(99999)
      end if
c----------------------------------------------------------------------
...
c----neccton
cc      if (i.eq.99999) then 
cc        write(6,*) 'before intrpl',is(1),is(99999)
cc      end if
	    if(is(i).ne.3) then
...
c	      call intrpl(px(i),py(i),hwave_vec,itype,hi)
              if (i.eq.99999) then 
                write(6,*) ' after intrpl',is(1),is(99999)
              end if
...
c******************************************************                
              ...
              else if(dep.gt.fcstdep3) then
                ui=u120(m,n)
                vi=v120(m,n)
              end if

c                 if(istoke.ne.0) then
c (M. De Dominicis)  stokes drift dispersed parcels 
c                 s_elev=exp(-2*wavenum(m,n)*dep)
c                 stokv_depth=stokv(m,n)
c                 stoku_depth=stoku(m,n)                
c                 ui=ui+stoku_depth
c                 vi=vi+stokv_depth
c                 endif

ccc              write(10,*) stoku(m,n),stokv(m,n),s_elev
ccc		 write(12,*) stoku_depth,stokv_depth
ccc              write(11,*) winx(m,n),winy(m,n)
              end if
...
c            elseif(iter.eq.2) then
c            m21=int(ppx+0.5d0)
c            n21=int(ppy+0.5d0)
c	    xdispl = (xds+xdc+xdd)
c	    ydispl = (yds+ydc+ydd)
c            ppx = px(i) + xdispl/delx
c            ppy = py(i) + ydispl/dely
c            endif
c----------------------------------------------------------------------
...
c-----neccton
   ...
   62         continue
      ...
      if (i.eq.99999) then 
c        write(6,*) 'SEGM ',nsgr,nseg,dd1min,dmin,px(99999),ppx,py(99999),ppy
        write(6,*) 'SEGM ',nsegt,nseg,dd1min,dmin,px(99999),ppx
        write(6,*) 'before vertd ',is(1),is(99999)
      end if
...
c----------------------------------------------------------------------
c     neccton project biodegradation
c----------------------------------------------------------------------
          do 666 i=l1,l2
            if ((is(i).eq.2).or.(is(i).eq.3)) then
              c1p(i)=c1p(i)*fDEGRAD
              c2p(i)=c2p(i)*fDEGRAD
            end if
  666     continue
...
c   neccton counts
            nea=nea+1
            if(is(i).lt.0) nen=nen+1
            if(is(i).eq.1) ne1=ne1+1
            if(is(i).eq.2) ne2=ne2+1
            if(is(i).eq.3) ne3=ne3+1
            if(is(i).eq.4) ne4=ne4+1
            if(is(i).gt.5) ne5=ne5+1
...
c----neccton
c        dbg1 = ' ' // en1ps // ' ' // en2ps // ' ' // en3ps // ' ' // en4ps
c        write(dbg1, '(I9)') en1ps,en2ps,en3ps,en4ps
c        write(dbg1, '*') en1ps,en2ps,en3ps,en4ps
c        dbg1 = str(en1ps,'(I9)') // str(en2ps,'(I9)')
c     &      // str(en3ps,'(I9)') // str(en4ps,'(I9)')
c----------------------------------------------------------------------
...
c----------------------------------------------------------------------
        if(nst.lt.nprs) go to 92
	      ...
	      write(a4,'(i4.4)') jtime
            ...
            open(nfile,file='output/'//fn(i))
            if(i.eq.1) write(nfile,801) vlunit  
            if(i.eq.2) write(nfile,802) vlunit   
            if(i.eq.3) write(nfile,803) vlunit 
            write(nfile,810) jtime,nbblr/volfac,vlunit,splat,splon,al5,
     &          pcevp,pcsrf,pcdsp,pccsttot,volsrf/volfac,tvolsrf/volfac        
   75     continue   
...
c
              mp=npl/2 + nint( (px(i)-xavg) * rax )
              np=npl/2 + nint( (py(i)-yavg) * ray )
...
c
	    ...
	    write(nfile,839) kount
            ...
            do 85 np=1,npl
              ...
              xx = xavg + (mp - npl/2) / rax
              yy = yavg + (np - npl/2) / ray
	        blon = glon(xx)
	        blat = glat(yy)
   ...
   85       continue
            ...
            do 87 k=1,nbooms
              ...
              write(nfile,846) bmlat1(k),bmlon1(k),bmlat2(k),bmlon2(k),
     &                         ibmeff(k)
   87       continue  
...
c
c  ------ neccton - no currents in SRF file
c			do m=m1,m2,nstep
c			do n=n1,n2,nstep
c			  write(nfile,851) glat(dfloat(n)),glon(dfloat(m)),
c     &							  uadv(m,n),vadv(m,n)
c			end do
c			end do
		  end if
...
c ----neccton
c      write(6,*) is(22222),is(33333),jtime,nea,ne1,ne2,ne3,ne4
      write(6,*) 'n=',nen,ne1,ne2,ne3,ne4,ne5,ne9

	    if(irestart.eq.0) then
            write(6,*) 'Simulation has now completed ',jtime,' hours'
          else 
            write(6,*) 'Simulation has now completed ',jtime,' hours',
     &               ' after restarting', dbg1   
          end if 
        end if 
...
c     Svitlana emodnet coastline October 2021 
c----------------------------------------------------------------------
	subroutine setcst(regn1)
	implicit real*8(a-h,o-z)
c      parameter(ndim=1000, nss=400000)
      parameter(ndim=10000, nss=400000)
c
...
c (M. De Dominicis) read Med coastline for Thyrrenian and Adriatic  	
	if(regn1.eq.'adri'.or.regn1.eq.'tyrr') then
	open(51,file='data/'//'medf.map',status='old')
        endif
...
c        Liubartseva Svitlana 2021.10.17   what is ieancr?!
c        write(6,*) 'iaencr=',iaencr
c        write(6,*) 'ibencr=',ibencr
c        write(6,*) 'icencr=',icencr
c        write(6,*) 'aloncorr=',aloncorr
        
        iaencr = 0
c ----neccton
c        write(6,*) 'FORCED iaencr=',iaencr
...
c----- neccton
cc            write(6,*) 'BEFORE setcst()'
cc            write(6,*) 'COASTAL segments  nsegt: ', nsegt
cc            write(6,*) 'COASTAL segments  impacted: ', nseg
cc            write(6,*) 'along1 along2 dlong ', along1,along2,dlong
cc            write(6,*) 'alatg1 alatg2 dlatg ', alatg1,alatg2,dlatg

c
      if(ind.eq.0) then

c        Liubartseva Svitlana 2021.10.17   medfcst1.d has already done!
         open(52,file='data/'//regn1//'cst1.d')
         call setcst(regn1)
ccccccccc        call setcst(regn1)
c----- neccton
cc            write(6,*) 'AFTER setcst()'
cc            write(6,*) 'COASTAL segments  nsegt: ', nsegt
cc            write(6,*) 'COASTAL segments  impacted: ', nseg
            nzerolat = 0
        ...
        do ns=1,nsegt
          ...
          if (bd1(ns,4).eq.0.d0) then nzerolat=nzerolat+1
          do j=1,3,2
...
cc            write(6,*) 'COASTAL segments  nsegt: ', nsegt
cc            write(6,*) 'COASTAL segments  impacted: ', nseg
c            write(6,*) 'xb xf yb yf ', xb,xf,yb,yf
c            write(6,*) 'x1 x2 y1 y2 ', x1,x2,y1,y2
cc            write(6,*) 'along1 dlong ', along1,dlong
cc            write(6,*) 'alatg1 dlatg ', alatg1,dlatg
c            stop
      return
      end             
...
c----------------------------------------------------------------------
	...
	implicit real*8(a-h,o-z)
      parameter(mm=600,nm=400,
     &          ntm=2000,npc=100000,nss=400000,msp=1200)
c     &          ntm=2000,npc=100000,nss=400000,npl=2000,msp=1200)
	save isub,dtc
c
      dimension temp(12)
      character regn1*4,empty*80
...
c
	...
	common /spill/  idd,imm,iyr,ispill,tstart,tcomp,x0,y0
c	common /blk3/ mmax,nmax,delx,dely,itype,pi,degrad,ihcst
	common /blk3/ mmax,nmax,delx,dely,itype,pi,degrad
	common /blk4/ along1,alatg1,along2,alatg2,dlong,dlatg
	common /temp/ m0,n0
...
c  strange ihcst; neccton
	...
	if(iwind.eq.27) dirr='fcst_data/E12/'
...
c---neccton
c	  write(6,*) 'nrecs = ',nrecs
c	  write(6,*) 'iwindrec = ',iwindrec
c	  write(6,*) 'Reading wind from file '//fn//', record ',iw
c DEBUGWIND	
...
c      implicit character*34(z)
      implicit real*8(a-h,o-z)
      character zd1*34,zd2*26
...
c neccton
c      open(72,file=dirr//fn)
c      open(72,file= 
c     * '/work/opa/sl35719/MEDSLIK_II_1.01/DATA/fcst_data_neccton/E12/'//
c     * fn)
       zd1='/work/opa/sl35719/MEDSLIK_II_1.01/'
       zd2='DATA/fcst_data_neccton/E12/'
      open(72,file=zd1//zd2//fn)
      read(72,*) dummy
	read(72,*) dummy
...
c      common /fcst/ sst(mm,nm),us(mm,nm),vs(mm,nm), ! Colucelli_bug_19/11/2013
...
c	    write(6,*) i,'   ',fcstfn(i),fcsttim(i)
        ...
        fn=fcstfn(1)
c ------neccton
c  write(6,*) 'Reading forecast currents from file ',fn
...
c----neccton
c DEBUGWIND
cc          write(6,*) 'fcstcur time = ',time
cc          write(6,*) 'ifile = ',ifile
cc          write(6,*) 'fcsttim(ifile) ',fcsttim(ifile)
c DEBUGWIND

	  ifile=ifile+1
	  
c DEBUGWIND
ccc          write(6,*) 'ifile=ifile+1 ',ifile
ccc          write(6,*) 'fcsttim(ifile) ',fcsttim(ifile)
c DEBUGWIND
...
c     &      dvs(mm,nm),du10(mm,nm),dv10(mm,nm),du30(mm,nm),dv30(mm,nm),
c     &	     du120(mm,nm),dv120(mm,nm)
c     &      ,wx(mm,nm),wy(mm,nm),dwx(mm,nm),dwy(mm,nm)	common /temp/ m0,n0
      common /fcst/ sst(mm,nm),wx(mm,nm),wy(mm,nm),us(mm,nm),
     &      vs(mm,nm),u10(mm,nm),v10(mm,nm),u30(mm,nm),v30(mm,nm),
     &      u120(mm,nm),v120(mm,nm),dsst(mm,nm),dwx(mm,nm),dwy(mm,nm),
     &      dus(mm,nm),dvs(mm,nm),du10(mm,nm),dv10(mm,nm),
...
c
     ...
     &.or.icurrents.eq.73.or.icurrents.eq.75.or.icurrents.eq.74
...
c DEBUGWIND
          write(6,*) 'fcstcur_1hr time = ',time
          write(6,*) 'ifile = ',ifile
          write(6,*) 'fcsttim(ifile) ',fcsttim(ifile)
          write(6,*) 'ifcstfn(ifile) ',ifcstfn(ifile)
          write(6,*) fcstfn(ifile)
c DEBUGWIND

	  ifile=ifile+1
	  
c DEBUGWIND
          write(6,*) 'ifile=ifile+1 ',ifile
          write(6,*) 'fcsttim(ifile) ',fcsttim(ifile)
          write(6,*) 'ifcstfn(ifile) ',ifcstfn(ifile)
          write(6,*) fcstfn(ifile)
c DEBUGWIND
...
c neccton
c      open(71,file=fcstcurdir//fn)
      open(71,file=
     * '/work/opa/sl35719/MEDSLIK_II_1.01/DATA/fcst_data_neccton/C24/'//
     * fn)
      read(71,*) dummy
	read(71,*) dummy
	read(71,*) alon1,alon2,alat1,alat2,mmax,nmax
...
c	first extrapolate to the whole grid (i.e. over the land points)
    ...
    1 continue
      ...
      do n=nzb2,nzf2
        ...
        do i=m-1,m+1
        do j=n-1,n+1
c  ---NECCTON condition order changed, main must be first
          if( i.ge.1 .and. j.ge.1 .and. i.le.mm .and. j.le.nm ) then
            if ( array(i,j) .ne. 0.d0 ) then
              sum=sum+array(i,j)
              no=no+1
            end if
          end if
